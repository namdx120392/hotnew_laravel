							...
							<table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
								...
								@if (!empty($categories))
									@foreach($categories as $key => $category)
										...
										<tr>
											<td><div class="line-cate">{{$line.$space}}</div> <a href="{{route('category-post-edit',$category->id)}}"> {{$category->name}}</a></td>
											<td class="status-td" id="{{$category->id}}">{!!$status!!}</td>
											<td class="action-td" role="gridcell" aria-describedby="jqgrid_act">
												<a href="{{route('category-post-edit',$category->id)}}" data-toggle="tooltip"
													title="Chỉnh sửa danh mục" class="btn btn-xs btn-default">
													<i class="fa fa-pencil"></i>
												</a>
												<a href="{{route('category-post-del',$category->id)}}" data-toggle="tooltip"
